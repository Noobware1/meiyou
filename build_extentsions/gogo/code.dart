...
// import 'gogo/extractor.dart';


const hostUrl = 'https://gogoanimehd.io';

main() {
  return Gogo();
}

class Gogo extends BasePluginApi {
  Gogo();
  ...
  Future<HomePage> loadHomePage(int page, HomePageRequest request) async {
    return HomePage(
        data: HomePageList(
            name: request.name,
            data: await getHomePageListData(page, request)));
  }
  ...
  Future<List<SearchResponse>> parseHomePage(String url) async {
    return ListUtils.mapList(
        (await MeiyouUtils.httpRequest(url: url, method: 'GET'))
            .document
  ...
  Future<List<SearchResponse>> search(String query) async {
    final doc = (await MeiyouUtils.httpRequest(
            url: '$hostUrl/search.html?keyword=${MeiyouUtils.encode(query)}',
            method: 'GET'))
        .document;
  ...
  Future<MediaDetails> loadMediaDetails(SearchResponse searchResponse) async {
    final animePage =
        (await MeiyouUtils.httpRequest(url: searchResponse.url, method: 'GET'))
            .document;
    ...
    for (var e in body.select('p.type')) {
      ...
      if (header == 'Plot Summary:') {
        ...
        media.genres = MeiyouUtils.selectMultiAttr(e.select('a'), 'title');
      } else if (header == 'Released:') {
        media.startDate = MeiyouUtils.toDateTime(StringUtils.toInt(
  ...
  ExtractorLink toExtractorLink(ElementObject it) {
    return ExtractorLink(
      ...
      url: MeiyouUtils.httpify(it.attr('data-video')),
    );
  }
}
